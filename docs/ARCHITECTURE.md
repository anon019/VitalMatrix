...
总计：**12-16天**完成MVP
# 当前 AI 架构说明（2026-09-18）

当前后端统一通过 Codex CLI 登录态调用 GPT 系列。饮食图片分析使用
`gpt-5.6-luna` + `medium`；每日健康建议、后续三餐建议和 AI 对话使用
`gpt-5.6-luna` + `low`；海报底图使用 `gpt-image-2`。数据库餐食分析字段为
`ai_analysis`。本文前面的多 Provider 片段是历史设计，不代表当前配置。

每日建议采用近 7 天明细与近 7/30/90 天统计，以及相邻 7 天、30 天对照；覆盖率和日期边界随数据一起传入模型。饮食规划加入实际菜品频次与餐次分布。图片核心识别仅使用必要的用户画像，长期菜品上下文用于每日建议和餐后三餐规划，避免增加首段识图等待。

模型调用具有总并发限制，后台长任务额外限流。餐食上传按用户、餐次、日期和图片摘要互斥；后台建议写入前核对识图版本和尝试时间，避免旧结果覆盖新分析。详见 [统计口径、API 契约与验证记录](2026-09-18-ai-context-and-nutrition-performance.md)。
